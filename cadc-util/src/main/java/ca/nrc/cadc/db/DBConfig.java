...
/**
 * Reads username/password pairs and (optionally) JDBC driver and connection URL
 * data from the ${user.home}/.dbrc file. The format of this file is:
 *
 * <pre>
 ...
 * 
 * <p>
 * To remove redundancy, one can have one line to list the driver and url for
 * all connections to a server with a line like:
 * </p>
 * 
 * <pre>
...
public class DBConfig implements Serializable {
    ...
    public DBConfig(File dbrc) throws FileNotFoundException, IOException {
        ...
        while (!eof) {
            ...
            if (!eof) {
                ...
                if ("".equals(s.trim()) || StringUtil.startsWith(s, COMMENT_INDICATOR)) {
                    continue;
                }
     ...
     * @throws NoSuchElementException if the server and database are found, usually
     *                                because they are missing from the .dbrc file
     */
    ...
    /**
     * TODO - This is very implementation specific, there must be a better way! TODO
     * - For now though this will need to do. We need to know which TODO -
     * configuration to load as some applications talk to multiple TODO - database
     * servers. A better way would be to classify the entries TODO - in the .dbrc
     * file. TODO - 2006.05.29 jenkinsd
     *
     * <p>Obtain whether the given server and database related to an entry that is that
     * of an IBM DB2 database. This implementation checks to see if the JDBC URL
     * contains a db2 String, so it's dependant on the URL containing the characters
     * 'db2'.
     *
     * @param server   server name from .dbrc file
     * @param database database name from .dbrc file
     ...
     *
     * <p>Obtain whether the given server and database related to an entry that is that
     * of a Sybase database. This implementation checks to see if the JDBC URL
     * contains a sybase String, so it's dependant on the URL containing the
     * characters 'sybase'.
     *
     * @param server   server name from .dbrc file
     * @param database database name from .dbrc file
