...
public class TestSchemeHandler implements SchemeHandler
{
    ...
    public List<URL> toURL(URI uri) throws IllegalArgumentException
    {
        ...
        // change protocol
        surl = surl.replace("test://", "http://");
        // change fragment to query string
        surl = surl.replace('#', '?');
        
        List<URL> ret = new ArrayList<URL>(1);
        try
        {
            ret.add(new URL(surl));
        }
        catch(MalformedURLException ex) { throw new RuntimeException("invalid URL: " + surl, ex); }
        return ret;
    }
