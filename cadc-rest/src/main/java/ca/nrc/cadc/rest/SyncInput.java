...
public class SyncInput {
    ...
    private void processMultiPart(FileItemIterator itemIterator)
        ...
        while (itemIterator.hasNext()) {
            ...
            if (item.isFormField()) {
                final List<String> existingItems = this.params.get(name);
                final String nextValue = Streams.asString(stream);
                final String[] paramValues;
                if (existingItems == null) {
                    paramValues = new String[] {nextValue};
                } else {
                    final String[] existingItemsArray = existingItems.toArray(new String[0]);
                    final int nextIndex = existingItemsArray.length;
                    paramValues = Arrays.copyOf(existingItemsArray, nextIndex + 1);
                    paramValues[nextIndex] = nextValue;
                }

                processParameter(name, paramValues);
            } else {
